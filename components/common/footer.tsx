...
const Footer = () => {
  ...
  const renderFooterContent = (): React.ReactNode => (
    <>
      <h1 className="font-medium text-3xl md:text-4xl text-center">Connect with me on social media.</h1>
      ...
      <h2 className="text-center text-sm sm:text-base mt-8">Built with ❤️ by Areeb Arshad</h2>
    </>
  );

  const { ref: footerRef } = MENULINKS[5];

  return (
    <footer className="w-full relative select-none bg-cover flex flex-col items-stretch" id={footerRef}>
      <img
