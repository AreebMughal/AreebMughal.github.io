...
const ProjectTile = ({ project, animationEnabled }: { project: IProject; animationEnabled: boolean }) => {
  ...
  const renderTechIcons = (techStack: string[]): React.ReactNode => (
    <div
      className={`
      ${styles.techIcons} w-1/2 h-[80%] absolute left-24 top-0 sm:flex items-center hidden
    `}
    >
  ...
  const renderDescription = (description: string): React.ReactNode => (
    <h2 className="text-lg z-10 tracking-wide font-medium" style={{ transform: 'translateZ(0.8rem)' }}>
      {description}
    </h2>
  );
