...
const ProjectsSection = ({ isDesktop }: IDesktop) => {
  ...
  const initRevealAnimation = (targetSectionRef: MutableRefObject<HTMLDivElement>): [GSAPTimeline, ScrollTrigger] => {
    const revealTl = gsap.timeline({ defaults: { ease: Linear.easeNone } });
    revealTl.from(targetSectionRef.current.querySelectorAll('.seq'), { opacity: 0, duration: 0.5, stagger: 0.5 }, '<');
  ...
  const initProjectsAnimation = (
    ...
    const sidePadding =
      document.body.clientWidth - targetSectionRef.current.querySelector('.inner-container').clientWidth;
    ...
    timeline.to(targetSectionRef.current, { x: width }).to(sectionTitleElementRef.current, { x: -width }, '<');
  ...
  useEffect(() => {
    ...
    if (isDesktop && matches) {
      ...
      const parentPadding = window.getComputedStyle(targetSectionRef.current).getPropertyValue('padding-left');
  ...
  const renderProjectTiles = (): React.ReactNode =>
    PROJECTS.map((project) => (
      <ProjectTile project={project} key={project.name} animationEnabled={horizontalAnimationEnabled}></ProjectTile>
    ));
